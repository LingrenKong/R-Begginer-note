...
# 坐标系统
# 图层语法
# ggplot(data = <DATA>) + #首先是数据
#   <GEOM_FUNCTION>(
#     mapping = aes(<MAPPINGS>), #图的轴
#     stat = <STAT>, #配置统计函数
#     position = <POSITION> #复杂变量要调整如何放图
#   ) +
#   <COORDINATE_FUNCTION> + #坐标轴转换
#   <FACET_FUNCTION> #图形分块

library(ggplot2)
library(car)
library(MASS) #这个包主要是要用他的数据，可以不使用library
# 加载数据用这个：data(UScereal,package = 'MASS')
###################################################
# A. ggplot 函数

# 1、quick plot function #这是ggplot简化版本的函数
# 散点图 
qplot(income,prestige, #xy数据
      xlab="Average Income", #轴坐标名
      ylab="Prestige Score", 
      geom=c("point", "smooth"), #点图+平滑拟合曲线 P.S.如果有大量整点数据重合，可以改用jitter，为散点添加随机波动
      data=Prestige) #car包的数据

# 分色散点图
qplot(income,prestige, 
      color = type, #分色用在有个类别变量的时候
      xlab="Average Income",
      ylab="Prestige Score",
      data=Prestige) 
